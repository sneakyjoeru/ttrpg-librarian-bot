const { Client, GatewayIntentBits, Partials, REST, Routes, SlashCommandBuilder, ChannelType, PermissionFlagsBits, ActivityType, AttachmentBuilder } = require('discord.js');
...
// --- INSTAGRAM INTERCEPTOR HELPERS ---
const INSTAGRAM_TYPING_INTERVAL = 10000;

async function sendRepostedMessage(message, content, attachments) {
...
async function handleInstagramMessage(message, instagramUrl, remadeContent) {
    ...
    try {
        ...
        if (downloadSuccess && attachments.length > 0) {
            await sendRepostedMessage(message, suppressedText, attachments);
        } else {
            await sendRepostedMessage(message, replacedText, []);
        }
...
client.on('messageCreate', async (message) => {
    ...
    if (instaMatch) {
        ...
        await handleInstagramMessage(message, instagramUrl, message.content);
        return;
    }
