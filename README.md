...
- **Scheduling Poll + Calendar Export**: `/schedule-poll [input]` turns a free-text spec like `Wednesday Friday 4` or `Wed Fri 18:00-22:00 4` into a live poll with one option per weekday for each of the next N weeks (up to 10 weeks / 20 dates). When there are more than 9 dates, random emojis are used for voting instead of 1️⃣..🔟. In an active campaign channel, the moment every member of the channel's campaign role has voted for the same date(s) (unanimous), the bot auto-generates and posts a Google-importable `.ics` calendar file. *(DM or Admin)*
