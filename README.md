...
- **Campaign Management**: Automated creation of active text channels with synced permissions, automatic role creation, and automated sync on channel renames. The channel's DM can rename their campaign via `/campaign-rename [new_name]` (keeps the creator-name + player-count suffix of the channel name and renames the linked campaign role to match).
