...
async function createSchedulePoll(interaction) {
    ...
    return interaction.followUp({
        content: `📅 Scheduling poll posted with ${options.length} date${options.length === 1 ? '' : 's'}${roleId ? '. When every member of this campaign role has voted for the same date(s), a Google-importable calendar (.ics) will be posted automatically.' : '.'}`,
        ephemeral: true
    }).catch(() => {});
}
...
// known role, checks whether every member of that role has voted for the
// SAME option(s) (unanimity). The first time this is true, generates an
// .ics with those unanimous options and posts it as a file attachment, then
// flips icsSent so subsequent vote churn doesn't re-emit.
async function handleSchedulingVoteChange(message, clientUserId) {
    ...
    for (const opt of state.options) {
        ...
        // Unanimous = every campaign-role member voted for this option.
        let allVoted = true;
    ...
    const description = `Auto-generated by Librarian Bot from poll ${message.id}. Confirmed by all ${memberIds.size} campaign-role member(s).`;
    const ics = buildIcs({
    ...
    try {
        await message.channel.send({
            content: `📅 **Consensus reached!** All ${memberIds.size} campaign-role member${memberIds.size === 1 ? '' : 's'} voted for ${confirmed.length === 1 ? 'this date' : 'these dates'}:\n${confirmed.map(o => `• ${o.label}`).join('\n')}\nHere is a Google-importable calendar (.ics) — open Google Calendar → Settings → Import & export → select this file.`,
            files: [{ attachment: buf, name: attachmentName }]
        });
        updateSchedule(message.id, { icsSent: true });
        console.log(`[Scheduling] Emitted ${attachmentName} for poll ${message.id} (${confirmed.length} unanimous option(s)).`);
    } catch (e) {
